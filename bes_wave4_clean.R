...
raw <- read_dta("data/bes/internet_panel/bes_wave4.dta")
raw[raw == 9999 | raw == 997] <- NA

df <- raw %>% 
  select(id, wt, turnoutUKGeneral, generalElectionVote, 
         partyId, trustMPs, trustYourMP, 
         ...
         changeEconomy, changeNHS, changeEducation, changeCostLive,
         immigrationLevel, euRefVote, immigEcon, immigCultural, 
         inequalityChange, inequalityChangeLarger, 
         inequalityChangeSmaller, inequalityGoodBad, satDemUK, 
         mpIndependent, mpLocalNational, mpRoots, mpParty, 
          localismImportance, oslaua, cci, ccinoIT, 
...
df[df == 9999] <- NA
df[df == 997] <- NA 
df$leave <- ifelse(df$euRefVote == 1, 1, 
                   ifelse(df$euRefVote == 2 | is.na(df$euRefVote), NA, 0))
df$inequalityGoodBad <- ifelse(df$inequalityGoodBad == 3, NA, df$inequalityGoodBad)
df$p_ethnicity <- as.numeric(df$p_ethnicity)
df <- df %>% 
  mutate(inequalityGood = case_when(
    inequalityGoodBad == 1 ~ 1, 
    inequalityGoodBad == 2 ~ 0, 
    TRUE ~ inequalityGoodBad
  ...
  mpRoots = as.numeric(mpRoots), 
  mpRootsRev = case_when(
    mpRoots == 7 ~ 1, mpRoots == 6 ~ 2, 
    mpRoots == 5 ~ 3, mpRoots == 4 ~ 4, 
    mpRoots == 3 ~ 5, mpRoots == 2 ~ 6, 
    mpRoots == 1 ~ 7, TRUE ~ mpRoots
...
#reverse order of some localism varaibles to make them all go in the asme direction


#read in geo lookup
geo <- read_csv("data/bes/internet_panel/bes_wave4_geography.csv") %>% 
  rename(oslaua_name = oslaua, region_name = gor, pcon_name = pcon)

df <- left_join(df, geo, by = "id")

#read in local authorty codes
...
#get the 2011 census for constituencies
cen <- census_11 %>% 
...
#make sectoral concentration variable using "industry" variables
sector <- census_11 %>% 
  dplyr::select(pano, region, ons_const_id, constituency_name,
                industry_agriculture:industry_other) %>% 
  mutate(natural_resources = industry_agriculture + industry_mining, 
         professional = industry_professional + industry_administrative, 
         education_social_work = industry_education + industry_social_work, 
         utilities = industry_electricity_supply + industry_water_supply, 
         transport_trade = industry_wholesale + industry_transport,
         finance_real_estate = industry_finance + industry_real_estate) %>% 
  dplyr::select(-industry_agriculture, -industry_mining, -industry_education,
                -industry_social_work, -industry_wholesale, -industry_transport, 
                -ends_with("supply"), -industry_finance, -industry_real_estate) %>% 
  pivot_longer(cols = industry_manufacturing:industry_other, 
               names_to = "sector", values_to = "value") %>% 
  mutate(sector = str_remove(sector, "industry_")) %>% 
  group_by(region, pano, ons_const_id, constituency_name) %>% 
  summarise(concen = Herfindahl(value, na.rm = TRUE))





write.csv(df, "data/bes/internet_panel/clean_data/bes_wave4_clean.csv")
#Note:
