...
non_localists <- raw11 %>% filter(belongLocal_scale == 0)
...
mod_local <- lmer(data = raw11, belongLocal_scale ~ p_edlevel +
                    age + male + p_socgrade + white_british +
                    p_gross_household + (1 | pcon))
mod_local_house <- lmer(data = raw11, belongLocal_scale ~ p_edlevel +
                          age + male + p_socgrade + white_british +
                          p_gross_household + own_house +
                          (1 | pcon))
mod_local_children <- lmer(data = raw11, belongLocal_scale ~ p_edlevel +
                             age + male + p_socgrade + white_british +
...
mod1_belong_demo <- lmer(data = raw11, belongLocal_scale ~ p_gross_household +
                           p_edlevel + age + male + p_socgrade + white_british + own_house +
                           (1 | pcon), weights = wt_full_)

mod1_belong_region <- lmer(data = raw11, belongLocal_scale ~ p_gross_household +
                             p_edlevel + age + male + p_socgrade + white_british + belongRegion + 
                             as.factor(region) + (1 | pcon))

mod1_belong_party <- lmer(data = raw11, belongLocal_scale ~ p_gross_household +
                            p_edlevel + age + male + p_socgrade + white_british +
                            as.factor(partyIdName) + (1 | pcon))

mod1_belong_context <- lmer(data = raw11, belongLocal_scale ~ p_gross_household + p_edlevel +
...
#RUN EFFICACY MODELS 
mod_effic1 <- lmer(data = raw11, belongLocal_scale ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
                     efficacyPolCare + (1 | pcon), weights = wt_full_)
mod_effic2 <- lmer(data = raw11, belongLocal_scale ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
                     efficacyNoMatter + (1 | pcon), weights = wt_full_)
mod_effic3 <- lmer(data = raw11, belongLocal_scale ~ p_gross_household +
                     p_edlevel + p_socgrade + white_british + as.factor(partyIdName) +
                     efficacyVoteEffort + (1 | pcon), weights = wt_full_)
mod_effic4 <- lmer(data = raw11, belongLocal_scale ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
...
mod_warm_syrian <- lmer(data = raw11, warmSyrians ~ p_gross_household + p_edlevel +
                          ...
                          ethnicity_white_british + rate2017 + belongLocal_scale + (1 | pcon), weights = wt_full_)
mod_warm_indian <- lmer(data = raw11, warmIndian ~ p_gross_household + p_edlevel +
                          age + male + white_british + p_socgrade +
                          belongLocal_scale + ethnicity_white_british + rate2017 + belongLocal_scale + (1 | pcon), weights = wt_full_)
...
mod_customs <- lmer(data = raw11, britCustoms ~ p_gross_household +
                      ...
                      belongLocal_scale + ethnicity_white_british + rate2017 + belongLocal_scale + (1 | pcon))
...
coef1 <- get_effic_est(mod_name = mod_warm_indian, var_name = "belongLocal_scale")
coef2 <- get_effic_est(mod_name = mod_warm_syrian, var_name = "belongLocal_scale")
coef3 <- get_effic_est(mod_name = mod_warm_eastern, var_name = "belongLocal_scale")
coef4 <- get_effic_est(mod_name = mod_customs, var_name = "belongLocal_scale")
coef5 <- get_effic_est(mod_name = mod_christian, var_name = "belongLocal_scale")
...
stargazer(mod_warm_syrian, mod_warm_indian, mod_warm_eastern, mod_customs, mod_christian,
          type = "text",
          keep = c("belongLocal_scale", "ethnicity_white_british", "rate2017"),
          dep.var.labels.include = FALSE,
...
mod4 <- lmer(data = raw15, voteCon ~ localEcon + p_gross_household + econGenRetro + econPersonalRetro +
               p_edlevel + age + male + p_socgrade + white_british + (1 | pcon))
mod5 <- lmer(data = raw11, vote_con ~ localEcon + belongLocal_scale + econGenRetro +
               econPersonalRetro + p_edlevel + age + male + p_socgrade + p_gross_household +
               white_british + (1 | pcon))
mod6 <- lmer(data = raw11, vote_con ~ localEcon * belongLocal_scale + econGenRetro + econPersonalRetro +
...
plot_mod6 <- plot(ggpredict(mod6, terms = c("localEcon", "belongLocal_scale", "
                                               econGenRetro [3]", "white_british [1]"))) +
  labs(x = "Local economic evaluation (low - high)") +
  theme(
...
raw11 %>%
  filter(!is.na(belongLocal_scale) & !is.na(p_edlevel) &
...
belong_local_plot <- region_plot(input_var = belongLocal_scale,
