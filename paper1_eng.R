...
mod_local <- lmer(data = eng11, belongLocal ~ p_edlevel +
                    age + male + p_socgrade + white_british +
                    p_gross_household + (1 | pcon))
mod_local_house <- lmer(data = eng11, belongLocal ~ p_edlevel +
                          age + male + p_socgrade + white_british +
                          p_gross_household + own_house +
                          (1 | pcon))
mod_local_children <- lmer(data = eng11, belongLocal ~ p_edlevel +
                             age + male + p_socgrade + white_british +
...
mod1_belong_demo <- lmer(data = eng11, belongLocal ~ p_gross_household +
                           p_edlevel + age + male + p_socgrade + white_british + own_house +
                           (1 | pcon), weights = wt_full_)

mod1_belong_region <- lmer(data = eng11, belongLocal ~ p_gross_household +
...
mod1_belong_party <- lmer(data = raw11, belongLocal ~ p_gross_household +
                            p_edlevel + age + male + belongRegion + p_socgrade + white_british +
                            as.factor(partyIdName) + (1 | pcon))

mod1_belong_context <- lmer(data = eng11, belongLocal ~ p_gross_household + p_edlevel +
...
context_plot <- cbind(
  ...
  filter(var %in% c(
    ...
    "scaleunem_const_rate", "population_density", "constituency_type.xCounty"
  )) %>%
  ...
  labs(
    ...
    caption = "Multilevel linear models, random effects at constituency level.
       ...
       Median income, unemployment, and house-price variables are scaled to have mean of 0 and s.d. of 1.
...
#RUN EFFICACY MODELS 
mod_effic1 <- lmer(data = eng11, belongLocal ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
                     efficacyPolCare + (1 | pcon), weights = wt_full_)
mod_effic2 <- lmer(data = eng11, belongLocal ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
                     efficacyNoMatter + (1 | pcon), weights = wt_full_)
mod_effic3 <- lmer(data = eng11, belongLocal ~ p_gross_household +
                     p_edlevel + p_socgrade + white_british + as.factor(partyIdName) +
                     efficacyVoteEffort + (1 | pcon), weights = wt_full_)
mod_effic4 <- lmer(data = eng11, belongLocal ~ p_gross_household + p_edlevel +
                     age + male + white_british + p_socgrade + as.factor(partyIdName) +
...
mod_warm_syrian <- lmer(data = eng11, warmSyrians ~ p_gross_household + p_edlevel +
                          age + male + white_british + p_socgrade +
                          ethnicity_white_british + rate2017 + belongLocal + (1 | pcon), weights = wt_full_)
mod_warm_indian <- lmer(data = eng11, warmIndian ~ p_gross_household + p_edlevel +
                          age + male + white_british + p_socgrade +
                          belongLocal + ethnicity_white_british + rate2017 + belongLocal + (1 | pcon), weights = wt_full_)
mod_warm_eastern <- lmer(data = eng11, warmEastern ~ p_gross_household + p_edlevel +
                           age + male + white_british + p_socgrade +
                           belongLocal + ethnicity_white_british + rate2017 + belongLocal + (1 | pcon), weights = wt_full_)
mod_customs <- lmer(data = eng11, britCustoms ~ p_gross_household +
                      p_edlevel + age + male + white_british + p_socgrade +
                      belongLocal + ethnicity_white_british + rate2017 + belongLocal + (1 | pcon))
mod_christian <- lmer(data = eng11, britChristian ~ p_gross_household +
                        p_edlevel + age + male + white_british + p_socgrade +
                        belongLocal + ethnicity_white_british + rate2017 + belongLocal + (1 | pcon))
...
mod1 <- lmer(data = eng11, vote_con ~ localEcon + p_gross_household +
               p_edlevel + age + male + p_socgrade + white_british + (1 | pcon))
mod2 <- lmer(data = eng15, voteCon ~ localEcon + p_gross_household +
               p_edlevel + age + male + p_socgrade + white_british + (1 | pcon))
mod3 <- lmer(data = eng11, vote_con ~ localEcon + p_gross_household +
               econGenRetro + econPersonalRetro +
               p_edlevel + age + male + p_socgrade + white_british + (1 | pcon))
mod4 <- lmer(data = eng15, voteCon ~ localEcon + p_gross_household + econGenRetro + econPersonalRetro +
               p_edlevel + age + male + p_socgrade + white_british + (1 | pcon))
mod5 <- lmer(data = eng11, vote_con ~ localEcon + belongLocal + econGenRetro +
               econPersonalRetro + p_edlevel + age + male + p_socgrade + p_gross_household +
               white_british + (1 | pcon))
mod6 <- lmer(data = eng11, vote_con ~ localEcon * belongLocal + econGenRetro + econPersonalRetro +
...
eng11 <- eng11 %>% mutate(`Own House` = own_house)
eng15 <- eng15 %>% mutate(`Own House` = own_house)

mod_home_2017_1 <- lmer(data = eng11, vote_con ~  own_house + econGenRetro + econPersonalRetro +
                          p_edlevel + age + male + p_socgrade + p_gross_household + white_british + (1 | pcon))
mod_home_2017_2 <- lmer(data = eng11, vote_con ~ localEcon * own_house + econGenRetro + econPersonalRetro +
                          p_edlevel + age + male + p_socgrade + p_gross_household + white_british + (1 | pcon))
mod_home_2019_1 <- lmer(data = eng15, voteCon ~ own_house + econGenRetro + econPersonalRetro +
                          p_edlevel + age + male + p_socgrade + p_gross_household + white_british + (1 | pcon))
mod_home_2019_2 <- lmer(data = eng15, voteCon ~ localEcon * own_house + econGenRetro + econPersonalRetro +
                          p_edlevel + age + male + p_socgrade + p_gross_household + white_british + (1 | pcon))
...
belongs <- eng11 %>% dplyr::select(starts_with("belong"), -starts_with("belongGroup"))
...
region_plot <- function(input_var, var_name){
  ...
  mean <- eng11 %>% summarise(mean = mean({{input_var}}, na.rm = TRUE)) %>% 
    pull()
  plot <- plot_df %>%
    ggplot() + 
...
mod_local <- lmer(data = eng11, strongestConnectionLocal ~ p_edlevel +
                    age + male + p_socgrade + white_british +
                    p_gross_household + (1 | pcon))
mod_local_house <- lmer(data = eng11, strongestConnectionLocal ~ p_edlevel +
                          age + male + p_socgrade + white_british +
                          p_gross_household + own_house +
                          (1 | pcon))
mod_local_children <- lmer(data = eng11, strongestConnectionLocal ~ p_edlevel +
                             age + male + p_socgrade + white_british +
                             p_gross_household + children_in_household +
                             (1 | pcon))
